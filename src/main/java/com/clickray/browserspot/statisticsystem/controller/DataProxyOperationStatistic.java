...
public interface DataProxyOperationStatistic extends JpaRepository<DataProxyOperation, Integer> {
    ...
    @Query("SELECT p FROM DataProxyOperation p WHERE p.createDate >= ?1 and p.createDate <=?2 and p.deleted = false and p.status = ?3")
    public List<DataProxyOperation> findByDateAndStatus(@Param("startTime") Timestamp startTime, @Param("endTime") Timestamp endTime, @Param("status") Integer status);

    @Query("SELECT p FROM DataProxyOperation p WHERE p.createDate >= ?1 and p.createDate <=?2 and p.deleted = false and p.status = ?3")
    public List<DataProxyOperation> findAutomateTestByDate(@Param("startTime") Timestamp startTime, @Param("endTime") Timestamp endTime, @Param("status") Integer status);

    @Query("SELECT p FROM DataProxyOperation p WHERE p.createDate >= ?1 and p.createDate <=?2 and p.deleted = false and p.status  < 0")
    public List<DataProxyOperation> findByDateFailedOperations(@Param("startTime") Timestamp startTime, @Param("endTime") Timestamp endTime);
